// https://dash.cloudflare.com/e2b3d1132ba12dc04ceae12c38d24f09/workers/services/view/petproject-56-buzz52-tg-github-trigger/production/settings
export default {
      ...
      try {
        ...
        if (payload.message && payload.message.text) {
          ...
          if (text === "/run" || text === "/check" || text === "/start") {
            ...
            replyMessage =
              "✅ Получен новый токен! Обновляю секрет и запускаю клики...";
          ...
          debugLog(
            `Делаю запрос к GitHub API... Репо: ${githubRepo}, Ветка: ${githubBranch}`,
          );
          ...
          if (githubRes.ok) {
            ...
            debugLog(
              `ОШИБКА GitHub: статус ${githubRes.status}, текст: ${githubResText}`,
            );
            console.error(
              `[ERROR] GitHub Response Error: ${githubRes.status} ${githubResText}`,
            );
        ...
        console.error(
          `[ERROR] КРИТИЧЕСКАЯ ОШИБКА в скрипте Cloudflare: ${e.message}`,
        );
