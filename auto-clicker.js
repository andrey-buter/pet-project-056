...
async function main() {
    ...
    let postsResponse;
    try {
        postsResponse = await fetch('https://api.buzz52.com/api/posts', { headers });
    } catch (e) {
    ...
    if (postsResponse.status === 401 || postsResponse.status === 403) {
        ...
        await sendTelegramMessage('⚠️ Токен Buzz52 протух (Expired 401)! Пожалуйста, обновите токен в GitHub Secrets (или другим способом).');
        process.exit(1);
    }
    ...
    for (const post of unclickedPosts) {
        ...
        try {
            const clickResponse = await fetch(`https://api.buzz52.com/api/posts/${post.post_id}/click`, {
                method: 'POST',
                headers: {
                    ...headers,
                    'content-length': '0'
            ...
            if (clickResponse.ok) {
                ...
                await sendTelegramMessage(`✅ Пост прокликался!\nID: ${post.post_id}\nURL: ${post.post_url}`);
            } else if (clickResponse.status === 401 || clickResponse.status === 403) {
                console.error('Token expired while clicking!');
                await sendTelegramMessage('⚠️ Токен Buzz52 протух (Expired) прямо во время кликов! Пожалуйста, обновите токен.');
                process.exit(1);
            } else {
                console.error(`Failed to click post ${post.post_id}. Status: ${clickResponse.status}`);
            }
        } catch (e) {
